...
def decorative_skeleton():
    # The anatomical landmarks are JOINT HEADS. Tails stay orientation-valid but
    # intentionally stop somewhere else than the child head, reproducing imported
    # rigs whose Blender display bones are disconnected/decorative.
    down=[0,1,0,0]  # 180 degrees around X: local +Y points toward world -Y.
    joints=[
      {'name':'root','parent':None,'head':[0,0,0],'tail':[0,.55,0],'rotation':[1,0,0,0]},
      {'name':'hips','parent':'root','head':[0,.1,0],'tail':[0,.24,0],'rotation':[1,0,0,0]},
      {'name':'thigh.L','parent':'hips','head':[.12,.1,0],'tail':[.12,-.03,0],'rotation':down},
      {'name':'shin.L','parent':'thigh.L','head':[.12,-.35,0],'tail':[.12,-.48,0],'rotation':down},
      {'name':'foot.L','parent':'shin.L','head':[.12,-.78,0],'tail':[.12,-.92,0],'rotation':down},
      {'name':'thigh.R','parent':'hips','head':[-.12,.1,0],'tail':[-.12,-.03,0],'rotation':down},
      {'name':'shin.R','parent':'thigh.R','head':[-.12,-.35,0],'tail':[-.12,-.48,0],'rotation':down},
      {'name':'foot.R','parent':'shin.R','head':[-.12,-.78,0],'tail':[-.12,-.92,0],'rotation':down},
    ]
    roles={'root':'root','hips':'hips','thigh_l':'thigh.L','calf_l':'shin.L','foot_l':'foot.L',
           'thigh_r':'thigh.R','calf_r':'shin.R','foot_r':'foot.R'}
    return {'joints':joints,'roles':roles,'source_fingerprint':digest(joints)}


class MorphologyRegressionTests(unittest.TestCase):
    def test_semantic_chain_scaling_ignores_decorative_tails(self):
        source=decorative_skeleton();before=body_profile(source)
    ...
    def test_fractional_final_key_gets_inclusive_scene_frame(self):
        ...
        self.assertAlmostEqual(exact,149.83,places=7);self.assertEqual(end,150)

    def test_near_integer_endpoint_is_not_extended_an_extra_frame(self):
        end,exact=inclusive_scene_end(1,149/30+1e-9,30)
        self.assertAlmostEqual(exact,150.00000003,places=6);self.assertEqual(end,150)

    def test_true_fractional_endpoint_ceil_is_intentional(self):
        end,_=inclusive_scene_end(1,4.9667,30);self.assertEqual(end,151)
