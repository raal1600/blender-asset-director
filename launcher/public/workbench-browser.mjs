...
export function browserView({ui,page,project,scene,locked,esc,b}) {
  ...
  return `<header class="browser-heading"><div><div class="eyebrow">Ingredients for ${esc(scene.name)}</div><h2 id="library-title">${activity==='all'?'Entire library':esc(scope.label)}</h2></div>${b('Done','browser-close',{},'ghost')}</header>
    ...
    <div class="browser-description"><p>${activity==='all'?'Showing all workflow types.':esc(scope.description)} ${source?'Original packages need reviewed intake before production import.':scene.stage==='world'?'Preview an asset, then add it to your world.':'Preview first, then Add to selection. Only Import creates scene objects.'}</p>${filtered?b('Clear filters','browser-reset',{},'ghost small'):''}</div><p id="browser-notice" class="note warn" role="alert" hidden></p>
