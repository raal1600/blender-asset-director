...
function itemView(a,{source,scene,locked,esc,b}) {
  ...
  return `<article class="browser-asset ${selected?'selected':''} ${motion?'motion':''}" data-item-id="${esc(a.id)}" data-kind="${esc(a.kind)}">
    <div class="browser-thumb"><div class="glyph" aria-hidden="true">${motion?'∿':'◇'}</div>${image?`<img data-${source?'source':'catalog'}-image="${esc(a.id)}" data-version="${esc(a.version)}" alt="Source reference: ${esc(name)}" hidden>`:''}<small>${image?'Checking reference…':motion?'Native motion':canPreview?'3D inspection in Blender':'Preview unavailable'}</small></div>
    ...
    <div class="asset-actions">${b('Details',detail,{id:a.id},'small')}${b('Preview…',source?'source-preview-detail':'catalog-preview-detail',{id:a.id,preview:'true'},'small',!canPreview||(source&&!a.available))}${b(selected?'Remove':'Add',source?'source':'catalog-select',{id:a.id},'small',locked||!!scene.candidate||(source&&!a.available&&!selected))}</div></article>`;
}
...
export function sourceDialog({source,scene,locked,esc,b}) {
  ...
  return {body:`<div class="asset-facts"><span class="asset-type" title="${esc(source.subcategory?.basis)}">${esc(typeLabel(source))}</span><span>${source.available?'Available package':'Unavailable'}</span></div><p>${source.fileCount} files · ${esc(humanBytes(source.bytes))} · Production use: ${esc(source.review||'UNREVIEWED')}</p>
    ...
    <details><summary>Source version and all package entry points</summary><pre>${esc(JSON.stringify({id:source.id,version:source.version,relative:source.relative,entrypoints:source.entrypoints},null,2))}</pre></details>`,buttons:`${members.length?b('Preview in Blender','asset-preview-open',{kind:'source',id:source.id,version:source.version},'primary',!source.available):''}${b(selected?'Remove from selection':'Add to selection','source',{id:source.id},'',locked||!!scene.candidate||(!source.available&&!selected))}`};
}
