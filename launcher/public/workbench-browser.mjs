...
function itemView(a,{source,project,scene,locked,esc,b}) {
  ...
  const label=world?source?(a.prepared?'Open prepared asset':'Prepare & add'):usage.action==='review'?'Review change':usage.action==='locate'?'View in scene':usage.action==='inspect'?'Inspect scene':'Add to scene':selected?'Remove':'Choose';
...
export function sourceDialog({source,scene,locked,esc,b}) {
  ...
  return {body:`<div class="asset-facts"><span class="asset-type" title="${esc(source.subcategory?.basis)}">${esc(typeLabel(source))}</span><span>${esc(libraryAvailability(source,{source:true}))}</span></div><p>${source.fileCount} files · ${esc(humanBytes(source.bytes))} · Original registry review: ${esc(source.review||'UNREVIEWED')}</p>
    ...
    <section class="asset-use"><h3>${source.prepared?'Prepared in My library':world?'Prepare, then add to your scene':'Use this original package'}</h3><p>${source.prepared?'A prepared catalog version is recorded for these exact source bytes. Open it to verify files, choose collections and add a new scene change.':world?'No download is needed: this package is already local. Prepare & add checks a separate copy in Blender and records verified files in the shared catalog. You will then review production source use and choose what to import. It never replaces your current scene.':'Movement needs native clip indexing and reviewed Action tools; selecting a package does not apply animation.'}</p>${world&&!supported?'<p class="note warn">This format needs reviewed conversion; it cannot be imported by this guided preparation.</p>':''}<p>Shared files are retained once for reuse across productions. Missing rights or external dependencies block use; they are not guessed.</p></section>
<details><summary>Source version, references and all package entry points</summary><p>Choosing a reference does not import geometry or grant rights.</p>${world?b(selected?'Remove reference':'Choose reference only','source',{id:source.id},'',blocked||(!source.available&&!selected)):''}<pre>${esc(JSON.stringify({id:source.id,version:source.version,relative:source.relative,entrypoints:source.entrypoints},null,2))}</pre></details>`,buttons:`${members.length?b('View in 3D','viewer-open',{kind:'source',id:source.id,version:source.version},'primary',!source.available)+b('Preview in Blender','asset-preview-open',{kind:'source',id:source.id,version:source.version},'',!source.available):''}${world?source.prepared?b('Open prepared asset','catalog-detail',{id:source.prepared.assetId,expectedVersion:source.prepared.assetVersion,back:'true'},'primary',blocked):b('Prepare & add','source-prepare-form',{id:source.id,version:source.version},'primary',blocked||!source.available||!supported):b(selected?'Remove from selection':'Add to selection','source',{id:source.id},'',blocked||(!source.available&&!selected))}`};
}
