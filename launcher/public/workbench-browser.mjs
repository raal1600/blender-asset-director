...
function itemView(a,{source,scene,locked,esc,b}) {
  ...
  return `<article class="browser-asset ${selected?'selected':''} ${motion?'motion':''}" data-item-id="${esc(a.id)}" data-kind="${esc(a.kind)}">
    ...
    <div class="asset-actions">${b('Details',detail,{id:a.id},'small')}${b('View 3D',source?'source-preview-detail':'catalog-preview-detail',{id:a.id,preview:'true'},'small',!canPreview||(source&&!a.available))}${b(selected?'Remove':scene.stage==='world'?'Choose':'Add',source?'source':'catalog-select',{id:a.id},'small',locked||!!scene.candidate||(source&&!a.available&&!selected))}</div></article>`;
}
export function browserView({ui,page,project,scene,locked,esc,b}) {
  ...
  return `<header class="browser-heading"><div><div class="eyebrow">Ingredients for ${esc(scene.name)}</div><h2 id="library-title">${activity==='all'?'Entire library':esc(scope.label)}</h2></div>${b('Done','browser-close',{},'ghost')}</header>
    ...
    <div class="browser-description"><p>${activity==='all'?'Showing all workflow types.':esc(scope.description)} ${source?'Original packages need reviewed intake before production import.':scene.stage==='world'?'Choose several assets; View 3D previews one asset at a time. Add each to world to see them together in the scene.':'Preview first, then Add to selection. Only Import creates scene objects.'}</p>${filtered?b('Clear filters','browser-reset',{},'ghost small'):''}</div><p id="browser-notice" class="note warn" role="alert" hidden></p>
