...
export function catalogDialog({asset,scene,locked,sourceReady,esc,b}) {
  ...
  const buttons=`${previewFiles.length?b('Preview in Blender','asset-preview-open',{kind:'catalog',id:asset.id,version:asset.version},'primary'):''}${b(selected?'Remove from selection':'Add to selection','catalog-detail-select',{id:asset.id},'',locked||!!scene.candidate)}${canWorld&&models.some(f=>f.toLowerCase().endsWith('.blend'))?b('Inspect collections','catalog-inspect',{id:asset.id},'',locked||!selected):''}${canWorld?b('Import into World','catalog-import',{id:asset.id},'',locked||!!scene.candidate||!selected||!models.length||!asset.policy?.eligible||!sourceReady):''}`;
