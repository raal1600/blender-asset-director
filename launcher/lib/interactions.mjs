...
export class Interactions {
 ...
 async runJob(jobId,elicit,signal){
  ...
  try{
   ...
   const job=await this.runtime.job(jobId);assert(job.id===jobId,'Harness job identity mismatch.');await this.store.bindJob(this.projectId,job);assert((await this.context()).scopeHash===gate.scopeHash&&(await this.store.verify(this.projectId)).ok,'Project inputs changed after confirmation; execution refused.',409);
