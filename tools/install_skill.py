...
def install(dest,update=False):
    ...
    try:
        ...
        data={'owner':'blender-asset-director','version':'0.2.0','files':hashes(stage)}
...
def self_test():
    with tempfile.TemporaryDirectory() as d:
        ...
        cmd=[sys.executable,str(dest/'scripts'/'director.py'),'--library',str(Path(d)/'library'),'plan','Present the supplied object clearly']
    ...
    print(json.dumps({'status':'PASS','checks':['fresh install','bundled runtime launch','role and license files','edited-file protection','uninstall preserves library']}))
