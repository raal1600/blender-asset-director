...
for(const kind of ['Meshes','Characters','Animations'])await fs.mkdir(path.join(root,'Database',kind),{recursive:true});
const secondInput=path.join(root,'Database/Meshes/Terrain/SecondSyntheticInputs');await fs.mkdir(secondInput,{recursive:true});
...
// This second package is registered, but deliberately NOT in the native catalog.
// The browser must prepare it through real inspection/intake before import.
const secondSource=(await app.store.scan()).sources.find(s=>s.entrypoints.some(f=>f.endsWith('/second.gltf')));
if(!secondSource)throw Error('Generated source registration failed');
