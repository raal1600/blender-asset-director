...
def main():
    ...
    with sync_playwright() as pw:
        ...
        def finish_job(previous=None):
            deadline=time.monotonic()+230
            while time.monotonic()<deadline:
                current=state()
                if not current['locked'] and current['project']['workbench']['scenes'][0]['candidate'] not in (None,previous):
        ...
        try:
            ...
            # Preview cancellation is read-only; explicit Add may pin the source,
            # but declining its permission must not prepare or execute a job.
            before=digest(session['projectManifest']);page.keyboard.press('Escape');idle()
            assert digest(session['projectManifest'])==before
            click('[data-action="catalog-preview-detail"][data-id="'+session['assetId']+'"]')
            click('[data-action="world-add"]')
            expect(page.locator('#dialog[open] #world-use-confirm')).not_to_be_checked()
            expect(page.locator('#dialog[open] [data-action="world-use-confirm"]')).to_be_disabled()
            assert not state()['project']['jobs'] and not scene()['candidate']
            capture('04-source-review-required')
            click('#dialog [data-action="close"]')
            assert not state()['project']['jobs'] and not scene()['candidate']
            report['checks'].append('Preview/Back is read-only; declining source permission starts no dependent job')
            click('.world-canvas-actions [data-action="browse-assets"]')
            click('.browser-locations [data-action="browser-location"][data-location="production"]')
            click('[data-item-id="'+session['assetId']+'"] [data-action="library-add"]')
            page.locator('#dialog[open] #world-use-confirm').check()
            click('#dialog[open] [data-action="world-use-confirm"]')
            accept=True
            ...
            click('[data-action="save-world"]');current=scene()
            ...
            row.get_by_role('button',name='Add to scene',exact=True).click();idle()
            expect(page.locator('#dialog[open] #prepare-confirm')).not_to_be_checked()
            ...
            page.locator('#dialog[open] #prepare-confirm').check()
            expect(page.locator('[data-action="source-prepare"]')).to_be_enabled()
            page.locator('#dialog[open] #prepare-confirm').uncheck()
            expect(page.locator('[data-action="source-prepare"]')).to_be_disabled()
            ...
            expect(page.locator('#dialog[open] #prepare-confirm')).not_to_be_checked()
            page.locator('#dialog[open] #prepare-confirm').check()
            click('[data-action="source-prepare"]');combined=finish_job()
            ...
            report['checks'].append('Production/library states; cancelled checkbox writes nothing; one explicit confirmation binds exact preparation and import')
            assert state()['sourceUse']['ready'] and combined['current']==first_cp['id']
            ...
            report['checks'].append('Explicit package add prepares and imports a second asset without intermediate save; original world and both real meshes remain')
            ...
            click('[data-action="world-add"]');second=finish_job();second_cp=next(c for c in second['checkpoints'] if c['id']==second['candidate'])
            assert second_cp['id']!=cp['id'];click('[data-action="world-undo"]')
            ...
            report.update(status='PASS',checkpoint_id=cp['id'],checkpoint_sha256=cp['sha256'],undone_checkpoint_id=second_cp['id'],browser=browser.version)
